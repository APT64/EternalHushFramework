...
def ClingyspiderDefaultCallback(cid):
    if eh.ui.GetEnv('CLSP_SURVEY_RUN', console_id= cid).lower() == 'true':
        eh.ui.Run('!survey -profile initialize', eh.RUN_NORMAL, console_id= cid)
        known = eh.ui.GetEnv('CLSP_SURVEY_KNOWN_TARGET', console_id= cid)
        if known.lower() == 'true':
            rescan = eh.ui.GetEnv("CLSP_SURVEY_RESCAN_TARGET")
            if rescan.lower() == 'true':
                eh.ui.Echo(f"CLSP_SURVEY_RESCAN_TARGET set to 'true', starting survey script", eh.ECHO_DEFAULT, console_id= cid)
                eh.ui.Run('!survey -profile informational', eh.RUN_NORMAL, console_id= cid)
            elif rescan.lower() == 'false':
                eh.ui.Echo(f"CLSP_SURVEY_RESCAN_TARGET set to 'false', skipping survey script", eh.ECHO_DEFAULT, console_id= cid)
