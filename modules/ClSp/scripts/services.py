...
def main(args):
    ...
    parsed_driverinfo = query_class_info(wmi_id, "Win32_Service", ["DisplayName", "Name", "ServiceType", 'PathName', 'AcceptPause', 'AcceptStop', 'State', 'StartMode', 'StartName'])

    xmlroot = ET.Element("TargetServices")

    for entry in parsed_driverinfo:
        physmedia_xml = ET.SubElement(xmlroot, "Service")
        ...
        ET.SubElement(physmedia_xml, "User").text =  str(entry['StartName'])
        ...
        eh.ui.Echo("User:\t\t" + str(entry['StartName']), eh.ECHO_DEFAULT)
