...
PyObject* import_rsa_key(PyObject* self, PyObject* args) {
	...
	{
		if (!NT_SUCCESS(import_pubkey(hAlgorithm, (PBYTE)(data.buf), data.len, &key)))
		{
			PyBuffer_Release(&data);
			Py_RETURN_NONE;
		}
